	...
	for benchmark in benchmarks:
		...
		for (dirpath, dirnames, filenames) in os.walk(project_dir + benchmark):
			for filename in filenames:
				...
				region_ratio = float(filename [begin : end])
		...
		f_KB.write("%d,%d,%d," % ((itlb_4KB["TotalAccesses"] + 3*itlb_4KB["TotalMisses"]), itlb_4KB["TotalMisses"], 3*itlb_4KB["TotalMisses"]))
		f_KB.write("%d,%d,%d,\n" % ((dtlb_4KB["TotalAccesses"] + 3*dtlb_4KB["TotalMisses"]), dtlb_4KB["TotalMisses"], 3*dtlb_4KB["TotalMisses"]))

		f_MB.write("%d,%d,%d," % ((itlb_4MB["TotalAccesses"] + 3*itlb_4MB["TotalMisses"]), itlb_4MB["TotalMisses"], 3*itlb_4MB["TotalMisses"]))
		f_MB.write("%d,%d,%d,\n" % ((dtlb_4MB["TotalAccesses"] + 3*dtlb_4MB["TotalMisses"]), dtlb_4MB["TotalMisses"], 3*dtlb_4MB["TotalMisses"]))
